...
source("Main.R")
settings$screens$end <- as.Date("2023-11-30")
settings$split$test_end <- as.Date("2023-11-30")
...
report <- function(dt, label, flag = NULL) {
  ...
  o <- d[eom >= as.Date("1981-01-01") & eom <= as.Date("2023-11-30")]
  f <- d
  po <- o[, .N, by=eom]; pf <- f[, .N, by=eom]
  cat(sprintf("%-44s | OOS 81-23: min=%4d med=%6.1f max=%4d | FULL 52-23: min=%4d med=%6.1f max=%4d\n",
